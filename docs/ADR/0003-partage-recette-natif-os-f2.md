...
- Image illustrative ~1080×1080 générée localement : photo principale plein cadre, bandeau bas « Recette envoyée via » + logo `favicon.svg` inline (pas d’overlay haut-droite ; copy image ≠ CTA install texte) — **pas** de fiche (titre / portions / ingrédients) sur l’image ; dégradation **texte seul** si fichiers non partageables. À l’import F2, génération/extraction d’image post-création reste async best-effort.
