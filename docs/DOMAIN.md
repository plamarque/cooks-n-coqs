...
16. **Échange texte F2 (partage natif)** : format d’échange sortant / entrant en texte clair (pas d’entité persistée dédiée). En-têtes `Titre:` / `Portions:` / `Ingrédients:` / `Étapes:` / `Source:` ; CTA install hors schéma d’en-têtes. À l’import, le même texte peut produire une `Recipe` via le pipeline texte existant ; la ligne CTA n’est pas une section de recette.
