...
16. **Échange texte F2 (partage natif)** : format d’échange sortant / entrant en texte clair (pas d’entité persistée dédiée). Sortant : titre nu en première ligne, ligne optionnelle `N portions`, en-têtes `Ingrédients:` / `Étapes:` / `Source:` ; CTA install hors schéma d’en-têtes. Entrant : même contrat **et** ancien wire `Titre:` / `Portions:`. À l’import, le même texte peut produire une `Recipe` via le pipeline texte existant ; la ligne CTA n’est pas une section de recette.
