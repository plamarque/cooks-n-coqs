...
| C | CRUD UI (liste, détail, formulaire) avec sauvegarde explicite, affichage et édition image ; après Enregistrer : vue détail + badge transitoire | Fait | — |
...
| K | Ajustement portions : recalcul quantités + UI détail (`FEATURE_PORTIONS_ENABLED`) | Fait | — |
...
| P | Partage natif OS (texte F2 ± image illustrative) ; F2 émis aux portions/quantités **affichées** ; retrait QR proximité | Fait | — |
